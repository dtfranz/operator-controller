...
- [ ] Remove SA annotations (`ServiceAccountNameKey`, `ServiceAccountNamespaceKey`) from `labels/labels.go` — **OPRUN-4630**

### Stream 3 — Authorization + Build tooling (Engineer C, no blockers)

- [ ] Remove `authorization/` package entirely (~700 lines) — **OPRUN-4631**
- [ ] Remove `k8s.io/kubernetes` dependency (require + replace in go.mod) — **OPRUN-4631**
- [ ] Remove `hack/tools/k8smaintainer/` — **OPRUN-4631**
- [ ] Remove `k8s-pin` Makefile target, simplify `verify` — **OPRUN-4631**

### Stream 4 — Integration: Appliers + Controllers + main.go (Engineer D, after streams 1-3)

- [ ] Remove `ServiceAccountValidator` reconciliation step — **OPRUN-4630**
- [ ] Remove `getUserInfo()` from Helm applier — **OPRUN-4630**
- [ ] Remove `getUserInfo()` from Boxcutter applier — **OPRUN-4630**
...
- [ ] Rewire `main.go`: remove all SA/synthetic/preflight wiring, always use controller's own SA — **OPRUN-4630**
